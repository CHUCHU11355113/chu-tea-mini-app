...
export const systemConfigs = mysqlTable("systemConfigs", {
  ...
  categoryIdx: index("idx_sys_category").on(table.category),
  codeIdx: index("idx_sys_code").on(table.code),
  enabledIdx: index("idx_sys_enabled").on(table.isEnabled),
}));
...
export const deliveryConfig = mysqlTable("delivery_config", {
  id: int("id").autoincrement().primaryKey(),
  storeId: int("storeId"), // null 表示全局配置
  
  // 配送方式开关
  deliveryEnabled: boolean("deliveryEnabled").default(true).notNull(), // 外卖开关
  pickupEnabled: boolean("pickupEnabled").default(true).notNull(), // 自提开关
  
  // 配送费设置
  deliveryFee: decimal("deliveryFee", { precision: 10, scale: 2 }).default("0.00").notNull(),
  freeDeliveryThreshold: decimal("freeDeliveryThreshold", { precision: 10, scale: 2 }), // 免配送费门槛
  
  // 起送金额
  minOrderAmount: decimal("minOrderAmount", { precision: 10, scale: 2 }).default("0.00").notNull(),
  
  // 配送范围（米）
  deliveryRadius: int("deliveryRadius").default(5000).notNull(),
  
  // 预计配送时间（分钟）
  estimatedDeliveryTime: int("estimatedDeliveryTime").default(30).notNull(),
  estimatedPickupTime: int("estimatedPickupTime").default(15).notNull(),
...
/**
 * 配置项表
 * 存储配置的具体选项（如糖度的具体选项：无糖、少糖、半糖等）
 */
export const configItems = mysqlTable("config_items", {
  id: int("id").autoincrement().primaryKey(),
  
  // 关联配置
  configId: int("configId").notNull(),
  parentId: int("parentId"), // 支持层级结构
  ...
  configIdIdx: index("idx_config_id").on(table.configId),
  parentIdIdx: index("idx_parent_id").on(table.parentId),
  enabledIdx: index("idx_ci_enabled").on(table.isEnabled),
}));

export type ConfigItem = typeof configItems.$inferSelect;
export type InsertConfigItem = typeof configItems.$inferInsert;

// ==================== 业务规则表 ====================

/**
 * 业务规则表
 * 存储所有业务规则，支持复杂的条件判断和动作执行
 */
export const businessRules = mysqlTable("business_rules", {
  ...
  // 规则类型
  ruleType: varchar("ruleType", { length: 50 }).notNull(), // coupon, points, member, marketing, pricing
  
  // 条件和动作
  conditions: json("conditions").notNull(), // 触发条件（支持AND/OR组合）
  actions: json("actions").notNull(), // 执行动作（顺序执行）
  
  // 优先级和互斥
  priority: int("priority").default(0).notNull(),
  mutexGroup: varchar("mutexGroup", { length: 50 }), // 互斥组
  ...
  ruleTypeIdx: index("idx_rule_type").on(table.ruleType),
  enabledIdx: index("idx_br_enabled").on(table.isEnabled),
 ...
 * 记录规则执行历史，用于分析和调试
 */
export const ruleExecutionLogs = mysqlTable("rule_execution_logs", {
  ...
  // 执行上下文
  contextType: varchar("contextType", { length: 50 }).notNull(), // order, user, product
  contextId: varchar("contextId", { length: 100 }).notNull(),
  
  // 执行结果
  isMatched: boolean("isMatched").notNull(),
  isExecuted: boolean("isExecuted").notNull(),
  result: json("result"),
  errorMessage: text("errorMessage"),
  ...
  ruleIdIdx: index("idx_log_rule_id").on(table.ruleId),
  contextIdx: index("idx_log_context").on(table.contextType, table.contextId),
  executedAtIdx: index("idx_log_executed_at").on(table.executedAt),
}));
